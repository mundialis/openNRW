...
############################################################################
#
# NAME:         unzip_convert_xyz2laz_pdal_parallel.sh
#
...
# Usage:
#   bash unzip_convert_xyz2laz_pdal_parallel.sh
########################################
...
for myzip in $(ls *.zip) ; do
  ...
  for xyz in $(ls *.xyz) ; do
    sem -j$PJOBS "echo \"Processing <$xyz>...\" ; sh ../convert_xyz2laz_pdal.sh $xyz ; rm -f $xyz ; echo \"<$xyz> done\""
  done
  sem --wait
